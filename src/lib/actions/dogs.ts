...
function getDogPayload(
  ...
  return {
    ...
    metro_station: optionalText(
      formData.get('metro_station'),
      120
    ),
...
export async function createDogAction(
  ...
  if (error) {
    ...
    throw new Error(
      'Не удалось создать карточку собаки'
    );
  }
...
export async function updateDogAction(
  ...
  if (error) {
    console.error('updateDogAction:', error);
    throw new Error(
      'Не удалось сохранить карточку собаки'
    );
  }

  revalidatePath('/owner');
  revalidatePath(`/owner/dogs/${dogId}/edit`);
...
export async function createProgressEntryAction(
  ...
  if (error) {
    ...
    throw new Error(
      'Не удалось добавить запись прогресса'
    );
  }
