...
export default async function AdminPage() {
  ...
  const [
    ...
    supabase
      ...
      .select(
        'id, public_name, city, verification_status, created_at'
      )
    ...
    supabase
      .from('owner_requests')
      .select('id, problem_type, status, created_at')
      .order('created_at', { ascending: false })
      .limit(20)
  ]);
  ...
  return (
    <main className="container-page py-10">
      ...
      <p className="section-subtitle">
        Пользователи, кинологи на проверке и последние заявки.
      </p>
        ...
        <section className="card">
          ...
          <div className="mt-5 space-y-4">
            {specialists?.map((specialist) => (
              <div
                className="rounded-3xl bg-white/70 p-5"
                key={specialist.id}
              >
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <b>{specialist.public_name}</b>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  <form action={approveSpecialistAction}>
                    <input
                      type="hidden"
                      name="specialist_id"
                      value={specialist.id}
                  ...
                  <form action={rejectSpecialistAction}>
                    <input
                      type="hidden"
                      name="specialist_id"
                      value={specialist.id}
            ...
            {!specialists?.length && (
              <p className="text-sm text-cocoa/60">
                Профилей специалистов пока нет.
              </p>
            )}
          </div>
        </section>
